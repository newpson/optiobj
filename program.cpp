...
std::shared_ptr<const Program> Program::from_shaders(
    const std::shared_ptr<const Shader> &shader_vertex,
    const std::shared_ptr<const Shader> &shader_fragment)
{
    return std::shared_ptr<const Program>(new Program(shader_vertex, shader_fragment));
}

Program::Program(const std::shared_ptr<const Shader> &shader_vertex,
                 const std::shared_ptr<const Shader> &shader_fragment)
...
void Program::set_uniform(const GLint location, const glm::vec2 &vector) const
{
    glUseProgram(m_id);
    glUniform2f(location, vector.x, vector.y);
}

void Program::set_uniform(const GLint location, const glm::mat4 &matrix) const
{
    glUseProgram(m_id);
